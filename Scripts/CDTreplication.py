...
- time         (start time of the trial in seconds, from a global clock)
- condition    ("0" or "90", the angular bias)
- accuracy     (1 if correct, else 0)
- prop         (the proportion of snippet used on that trial)
- rolling_acc  (rolling average accuracy for that condition, last 10 trials)
...
data_path = "/Users/simonknogler/Desktop/PhD/Control Detection Task"
...
# A global clock to record the start time of each trial
...
def generate_OU_snippet(n_frames=300, theta=0.2, sigma=2.0, dt=1.0):
    ...
    return v  # each row is the displacement for that frame

snippet_length = 300  # 5 seconds @ 60Hz
num_snippets = 2000   # For a real experiment, generate more (e.g., 50,000) offline
library = []
...
def random_positions():
    """Positions for square & dot within ±250 px, at least 200 px apart."""
    while True:
...
#############################################
# For 0° and 90° conditions, we track consecutive correct answers.
# If 2 consecutive correct => proportion += step (harder).
# If 1 incorrect => proportion -= step (easier).
...
#############################################
# 5. Condition List (20 Trials)
#############################################
# 10 trials at 0°, 10 trials at 90°, randomly shuffled
...
instruction_text.text = (
    ...
    "We have 10 trials at 0° and 10 at 90° (random order). Move your mouse for 5s.\n"
    "One shape is partially under your control (mixed with a random snippet),\n"
    "the other shape is purely the snippet. After each trial, choose which shape\n"
    "you controlled more, then rate your confidence. The staircase adjusts difficulty.\n\n"
...
for trial_info in conditions_list:
    ...
    # Decide proportion for this trial (before updating)
    if angle_bias == 0:
    ...
    if angle_bias == 0:
        ...
        print(f"Trial {trial_count} (0°) => new prop={prop_0deg:.2f}, roll_acc={new_roll_acc:.2f}")
    else:
        if accuracy == 1:
        ...
        print(f"Trial {trial_count} (90°) => new prop={prop_90deg:.2f}, roll_acc={new_roll_acc:.2f}")

    # --- Save columns ---
    thisExp.addData("time", trial_time)                 # start time of the trial
    thisExp.addData("condition", str(angle_bias))       # "0" or "90"
    thisExp.addData("accuracy", accuracy)               # 1 or 0
    thisExp.addData("prop", prop_trial)                 # proportion used this trial
...
# End
print("Final staircase values:")
print(f"0° condition: {prop_0deg:.2f}")
print(f"90° condition: {prop_90deg:.2f}")

instruction_text.text = "Thank you! Experiment complete."
instruction_text.draw()
win.flip()
core.wait(2.0)

thisExp.saveAsWideText(os.path.join(data_path, filename + ".csv"))
thisExp.saveAsPickle(os.path.join(data_path, filename))
win.close()
core.quit()
